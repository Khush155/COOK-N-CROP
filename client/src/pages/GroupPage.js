...
const GroupPage = () => {
  ...
  const fetchGroupDetails = useCallback(async () => {
    try {
      ...
      if (isAuthenticated) {
        setIsMember(
          groupData.members &&
            groupData.members.some((memberId) => memberId === user.id)
        );
        setHasRequestedToJoin(
          groupData.joinRequests &&
            groupData.joinRequests.some((reqUser) => reqUser._id === user.id)
        );
      }
    } catch (err) {
  ...
  const isMod =
    group &&
    ((group.moderators &&
      group.moderators.some((mod) => mod._id === user?.id)) ||
  ...
  const handleJoinLeave = async () => {
    ...
    try {
      ...
      if (group.isPrivate && !isMember) {
        ...
        setSnackbar({
          open: true,
          message: res.isMember ? "Joined group!" : "Left group.",
          severity: "success",
        });
      }
    } catch (err) {
  ...
  const handleUpvote = async (postId) => {
    ...
    updatedPost.upvotes = hasUpvoted
      ? updatedPost.upvotes.filter((id) => id !== user.id)
      : [...updatedPost.upvotes, user.id];
    updatedPost.voteScore =
      (updatedPost.voteScore || 0) + (hasUpvoted ? -1 : 1);
        ...
        sx={{
          ...
          backgroundImage: `url(${
            group.coverImage.startsWith("http")
              ? group.coverImage
              : `${process.env.REACT_APP_API_URL}${group.coverImage}`
          })`,
          color: "#fff",
          display: "flex",
          alignItems: "flex-end",
                              ...
                              <Avatar
                                src={
                                  post.user?.profilePic
                                    ? `${process.env.REACT_APP_API_URL}${post.user.profilePic}`
                                    : undefined
                                }
                ...
                {group.creator && (
                  ...
                  >
                    ...
                    <Typography
                      ...
                      to={`/user/${group.creator.username}`}
                      sx={{
                        fontWeight: "bold",
                        textDecoration: "none",
                        color: "text.primary",
                      ...
                      {group.moderators.map((mod) => (
                        ...
                        >
                          ...
                          <Avatar
                            ...
                            src={
                              mod.profilePic &&
                              mod.profilePic.startsWith("http")
                                ? mod.profilePic
                                : mod.profilePic
                                ? `${process.env.REACT_APP_API_URL}${mod.profilePic}`
                                : undefined
                            }
                          >
                  ...
                  {group.moderators.map((mod) => (
                    ...
                    >
                      ...
                      <Avatar
                        ...
                        src={
                          mod.profilePic && mod.profilePic.startsWith("http")
                            ? mod.profilePic
                            : mod.profilePic
                            ? `${process.env.REACT_APP_API_URL}${mod.profilePic}`
                            : undefined
