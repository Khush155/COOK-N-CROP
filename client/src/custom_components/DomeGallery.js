...
export default function DomeGallery({
  ...
  grayscale = true,
  onImageClick, // Add custom click handler prop
  autoRotate = true, // Add auto-rotate feature
  autoRotateSpeed = 0.2, // Speed of auto-rotation
  showVisualCues = true // Show visual cues for interactivity
}) {
  ...
  const autoRotateRAF = useRef(null); // Ref for auto-rotation
  ...
  const startAutoRotate = useCallback(() => {
    ...
    const rotate = () => {
      // Always auto-rotate regardless of interactions
  ...
  useEffect(() => {
    ...
    return () => {
      // Clean up auto-rotation on unmount
      stopAutoRotate();
    };
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        // Don't stop auto-rotation when user starts dragging
        stopInertia();
        const evt = event;
  ...
  // Remove mouse enter/leave events that pause/resume auto-rotation
  // The gallery will now keep rotating continuously
  ...
  const onTileClick = useCallback(
    e => {
      ...
      // Check if custom click handler is provided
      if (onImageClick) {
        const parent = e.currentTarget.parentElement;
        const imageSrc = parent.dataset.src;
        const imageId = parent.dataset.id; // Get the ID if available
        onImageClick({ src: imageSrc, id: imageId });
        return;
      }
          ...
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => (
              <div
                ...
                className="item"
                data-src={it.src}
                data-id={it.id} // Add ID data attribute
                data-offset-x={it.x}
                data-offset-y={it.y}
